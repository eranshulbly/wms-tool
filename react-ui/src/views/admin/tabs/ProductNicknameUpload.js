...
// ---------------------------------------------------------------------------
// ProductNicknameUpload
// ---------------------------------------------------------------------------
const ProductNicknameUpload = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await api.post('admin/product-nickname', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      setUploadResult(res.data);
      if (res.data.success) {
        showSnack(
          `Done — ${res.data.updated} updated, ${res.data.skipped} skipped`,
          'success'
        );
  ...
  const saveEdit = async (productId) => {
    ...
    try {
      const res = await api.patch(`admin/products/${productId}/nickname`, { nickname: editValue });
      if (res.data.success) {
        setProducts((prev) =>
          prev.map((p) =>
            p.product_id === productId ? { ...p, nickname: editValue } : p
          )
        );
        showSnack('Nickname updated', 'success');
        cancelEdit();
      } else {
                  ...
                  <Table size="small">
                    <TableHead>
                      <TableRow>
                        ...
                        <TableCell><strong>Product String</strong></TableCell>
                    ...
                    <TableBody>
                      {uploadResult.errors.map((e, i) => (
                        <TableRow key={i}>
                          ...
                          <TableCell>{e.product_string || '—'}</TableCell>
        ...
        <Box className={classes.tableToolbar}>
          ...
          <Box display="flex" alignItems="center" style={{ gap: 8 }}>
            <TextField
              size="small"
              variant="outlined"
              placeholder="Search by name or product string…"
              value={search}
            ...
            <Tooltip title="Refresh">
              <IconButton size="small" onClick={() => fetchProducts(search)}>
                <IconRefresh size={18} />
              </IconButton>
            </Tooltip>
          </Box>
          ...
          <Table stickyHeader size="small">
            <TableHead>
              <TableRow>
                ...
                <TableCell style={{ fontWeight: 600 }}>Product String</TableCell>
                <TableCell style={{ fontWeight: 600 }}>Description</TableCell>
                <TableCell style={{ fontWeight: 600 }}>Nickname</TableCell>
                ...
                products.map((product, idx) => {
                  ...
                  return (
                    <TableRow key={product.product_id} hover>
                      ...
                      <TableCell>{product.name || <span style={{ color: '#bdbdbd' }}>—</span>}</TableCell>
                      <TableCell>
                        {product.product_string
                          ? <code style={{ fontSize: '0.8rem' }}>{product.product_string}</code>
                          : <span style={{ color: '#bdbdbd' }}>—</span>}
                      </TableCell>
                      <TableCell>
                        <Typography variant="body2" color="textSecondary">
                          {product.description || <span style={{ color: '#bdbdbd' }}>—</span>}
                        </Typography>
                      </TableCell>

                      {/* Nickname — view or edit */}
                      <TableCell>
                        {isEditing ? (
                          <TextField
                            autoFocus
                            ...
                            onKeyDown={(e) => handleEditKeyDown(e, product.product_id)}
                          ...
                          <Typography
                            variant="body2"
                            style={{
                              color: product.nickname ? 'inherit' : '#bdbdbd',
                              fontStyle: product.nickname ? 'normal' : 'italic',
                            }}
                          >
                            {product.nickname || 'Not set'}
                          </Typography>
                        )}
                      </TableCell>
...
export default ProductNicknameUpload;
