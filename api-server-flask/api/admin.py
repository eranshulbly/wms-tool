...
class RoleManagementView(SecureView):
    ...
    def index(self):
        roles = mysql_manager.execute_query(
            "SELECT role_id, name, description, all_warehouses, eway_bill_admin, eway_bill_filling, supply_sheet, analytics FROM roles ORDER BY name"
        )
    ...
    def create(self):
        ...
        if request.method == 'POST':
            ...
            analytics = request.form.get('analytics') == '1'
            ...
            with mysql_manager.get_cursor() as cursor:
                cursor.execute(
                    "INSERT INTO roles (name, description, all_warehouses, eway_bill_admin, eway_bill_filling, supply_sheet, analytics) VALUES (%s,%s,%s,%s,%s,%s,%s)",
                    (name, description, all_warehouses, eway_bill_admin, eway_bill_filling, supply_sheet, analytics)
                )
    ...
    def edit(self, role_id):
        ...
        role = mysql_manager.execute_query(
            "SELECT role_id, name, description, all_warehouses, eway_bill_admin, eway_bill_filling, supply_sheet, analytics FROM roles WHERE role_id=%s", (role_id,)
        )
        if not role:
        ...
        if request.method == 'POST':
            ...
            mysql_manager.execute_query(
                "UPDATE roles SET description=%s, all_warehouses=%s, eway_bill_admin=%s, eway_bill_filling=%s, supply_sheet=%s, analytics=%s WHERE role_id=%s",
                (description, all_warehouses, eway_bill_admin, eway_bill_filling, supply_sheet, analytics, role_id), fetch=False
            )
            # Replace permissions
            mysql_manager.execute_query(
